...
La recomendación para este caso, basada en los resultados obtenidos a lo largo del laboratorio, conduce a la selección del algoritmo de ordenamiento Merge Sort. Este algoritmo permite mantener una única implementación en Tamiza, aunque el centro decida cambiar el canal de entrada, ya que su comportamiento no depende de que los datos lleguen aleatorios, casi ordenados o en orden inverso.

Esta conclusión se sustenta en base a uno de los resultados obtenidos al evaluar por error Insertion Sort y Merge Sort en los tres escenarios (A, B y C), aunque inicialmente solo se solicitaba evaluar el escenario A. Gracias a esta evaluación adicional se pudo observar que, independientemente del escenario, Merge Sort presentaba mejores resultados en comparación con Insertion Sort en la mayoría de los casos. Insertion Sort solo presentaba cierta ventaja para entradas pequeñas, como n = 100, pero eso puede llegar a ser poco frecuente en el caso de Tamiza, teniendo en cuenta que actualmente se manejan aproximadamente 1.200.000 registros. Por lo tanto, el criterio utilizado es priorizar un algoritmo cuyo desempeño sea menos dependiente del escenario de entrada y permita utilizar una sola implementación ante posibles cambios en el canal. 

Para estimar si Tamiza puede cumplir con la ventana de cuatro horas con 1.200.000 registros, se utilizó como referencia los resultados obtenidos con n = 6400. En el caso del Insertion Sort, en el escenario A se obtuvo un tiempo de ejecución de 1.786217 segundos. Dado que su complejidad temporal media es Θ(n²), al extrapolar este crecimiento a 1.200.000 registros se obtiene un tiempo aproximado de 17.4 horas, lo que significa que no cumpliría el plazo establecido (4 horas). En el caso del Merge Sort, cuya complejidad temporal es Θ (n log n), la extrapolación a partir de la medición con n = 6400 arroja un tiempo aproximado de 6 segundos. Los valores son estimaciones basadas en extrapolaciones y no son mediciones directas haciendo uso de 1.200.000 de registros. 

La propuesta de adquirir un servidor con el doble de velocidad no resolvería el problema principal de Tamiza. Esto se puede observar en el gráfico de la sección 4.2, donde, para una entrada de 6.400 registros, el Insertion Sort tarda más de 2 segundos, mientras que el Merge Sort tarda considerablemente menos tiempo. Además, aunque el servidor con el doble de rapidez pudiese reducir el tiempo de ejecución aproximadamente a la mitad, no cambiaría la complejidad Θ(n²) del Insertion Sort.

Por último, además del tiempo de ejecución, hay que tener en cuenta la memoria adicional que utiliza el Merge Sort durante el proceso de ordenación, ya que requiere dividir los datos y, posteriormente, fusionar las partes ordenadas. Por lo tanto, su implementación debe tener en cuenta este consumo adicional de memoria como parte de los recursos necesarios para lograr la mejora de rendimiento. 
